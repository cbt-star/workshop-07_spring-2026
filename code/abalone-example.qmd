...
# create new object from original data set 
abalone_clean <- abalone |> 
  # clean names to make more readable
  clean_names()  |> 
  # choose the columns to keep in clean data frame
...
# base layer: ggplot
ggplot(data = abalone_clean, # use clean data frame
       aes(x = mean_ph, # x-axis: pH
           y = change_in_area)) + # y-axis: shell SA change
# 1st layer: geom_point to represent individual observations 
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
`x`, `predicted`, `std.error`, `conf.low`, `conf.high`, `group` (not going to worry about `group` for now)
...
**This is a ggeffects object and also a data frame.**  
...
**At pH = 8, the model predits abalone shell growth of 5.36 [95% CI: 5.08, 5.64 mm^2^ d^_1.**
